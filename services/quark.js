...
function init() {
  ...
  _quarkchainWeb2.default.injectWeb3(web3, serverUrl);
...
function callFunction(functionName, ...inputArgs) {
  ...
  var abiMethods = quarkParam.movieAbi.filter(i => i.name && i.type === "function").sort((a, b) => a.name.localeCompare(b.name));
